    ...
    .select(`
      *,
      ...
      items:standing_order_items(
        ...
        product:products(
          id, name, price, unit, product_number
        )
      )
    `)
...
// Get delivery day from either field name
function getDeliveryDay(order: any): string | null {
  return order.delivery_day || order.delivery_days || null
}
...
export default async function StandingOrdersPage() {
  ...
  const activeOrders  = standingOrders.filter((so: any) => so.active).length
  const totalCustomers = new Set(standingOrders.map((so: any) => so.customer_id)).size
  const totalItems    = standingOrders.reduce((sum: number, so: any) => sum + (so.items?.length || 0), 0)
      ...
      <div className="mb-8">
        ...
        <div className="flex justify-between items-start flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-600">Manage recurring weekly orders for customers</p>
          </div>
          <Link
            ...
            <TableBody>
              {standingOrders.length === 0 ? (
                ...
                standingOrders.map((order: any) => {
                  const day = getDeliveryDay(order)
                    ...
                    <TableRow key={order.id}>
                      <TableCell>
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{order.customer?.contact_name}</p>
                      ...
                      <TableCell>
                        <span className="text-sm">{formatDate(order.next_generation_date)}</span>
                      </TableCell>
                      <TableCell>
                        <span className="text-sm text-gray-500">{formatDate(order.last_generated_date)}</span>
                      </TableCell>
                      <TableCell>
                        {order.active ? (
