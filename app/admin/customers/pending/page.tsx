...
export default async function PendingCustomersPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <Link href="/admin" className="flex items-center gap-1 text-sm mb-5 hover:opacity-80" style={{ color: '#CE1126' }}>
        <ArrowLeft className="h-4 w-4" /> Back to Admin
      </Link>

      <div className="flex items-center gap-3 mb-6">
        <Clock className="h-6 w-6 text-orange-500" />
        <h1 className="text-2xl font-bold" style={{ color: '#006A4E' }}>Pending Approvals</h1>
        {count > 0 && (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {customer.abn && <p className="text-xs text-gray-400">ABN: {customer.abn}</p>}
                    {customer.delivery_notes && (
                      <p className="text-xs text-blue-600 mt-1 italic">Notes: {customer.delivery_notes}</p>
                    )}
